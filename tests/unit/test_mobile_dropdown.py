...
class TestMobileDropdownModule:
    ...
    def test_css_mobile_positioning(self):
        """Test that CSS properly positions dropdown for mobile"""
        template_path = os.path.join(
            os.path.dirname(__file__),
            '..', '..',
            'templates', 'index.html'
        ...
        assert 'position: fixed !important' in content
        assert 'left: 50% !important' in content
        assert 'bottom: 20px !important' in content
        assert 'transform: translateX(-50%) !important' in content
        assert 'width: 280px !important' in content
        assert 'max-width: 90vw !important' in content
